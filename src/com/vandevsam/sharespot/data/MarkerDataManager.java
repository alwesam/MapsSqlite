...
public class MarkerDataManager {
	...
	SQLiteDatabase db;
	String[] cols = { MySQLHelper.TITLE, MySQLHelper.SNIPPET,
			MySQLHelper.POSITION, MySQLHelper.GROUPS, MySQLHelper.STATUS };
	...
	public void deleteMarker(MyMarkerObj n) {
		db.delete(MySQLHelper.MARKER_TABLE,
				MySQLHelper.POSITION + " = '" + n.getPosition() + "'", null);
	}
	...
	public void deleteMarkerGroup(String group) {
		db.delete(MySQLHelper.MARKER_TABLE, MySQLHelper.GROUPS + " = '" + group
				+ "'", null);
	}

	public List<MyMarkerObj> getAllMarkers() {
		List<MyMarkerObj> markers = new ArrayList<MyMarkerObj>();
		Cursor cursor = db.query(MySQLHelper.MARKER_TABLE, cols, null, null,
				null, null, null);
		cursor.moveToFirst();
	...
	private String buildQuery(List<String> col_string) {
		...
		for (int i = 0; i < col_string.size(); i++)
			sel[i] = "SELECT * FROM locations where groups = '"
					+ col_string.get(i) + "'";
	...
	public MyMarkerObj getSelectMarker(String pos) {
		Cursor cursor = db.query(MySQLHelper.MARKER_TABLE, cols, "position = '"
	...
	private MyMarkerObj cursorToMarker(Cursor cursor) {
		...
		m.setSnippet(cursor.getString(cursor
	...
	/**
	 * Compose JSON out of SQLite records that are not synced with remote db
	 * 
	 * @return JSON String
	 */
	public String composeJSONfromSQLite() {
		...
		String selectQuery = "SELECT  * FROM locations where updateStatus = '"
				+ "no" + "'";
		Cursor cursor = db.rawQuery(selectQuery, null);
		if (cursor.moveToFirst()) {
			do {
				...
				map.put(MySQLHelper.COLUMN_ID, cursor.getString(cursor
						.getColumnIndex(MySQLHelper.COLUMN_ID)));
				map.put(MySQLHelper.TITLE, cursor.getString(cursor
						.getColumnIndex(MySQLHelper.TITLE)));
				map.put(MySQLHelper.SNIPPET, cursor.getString(cursor
						.getColumnIndex(MySQLHelper.SNIPPET)));
				map.put(MySQLHelper.POSITION, cursor.getString(cursor
						.getColumnIndex(MySQLHelper.POSITION)));
				map.put(MySQLHelper.GROUPS, cursor.getString(cursor
						.getColumnIndex(MySQLHelper.GROUPS)));
	...
	public void updateSyncStatus(String id, String status) {
		String updateQuery = "Update locations set updateStatus = '" + status
				+ "' where id=" + "'" + id + "'";
	...
	public List<String> getMarkerDetails(String pos) {
		...
		String selectQuery = "SELECT  * FROM locations where position = '"
				+ pos + "'";
		Cursor cursor = db.rawQuery(selectQuery, null);
		if (cursor.moveToFirst()) {
			details.add(cursor.getString(cursor
					.getColumnIndex(MySQLHelper.TITLE)));
			details.add(cursor.getString(cursor
					.getColumnIndex(MySQLHelper.SNIPPET)));
			details.add(cursor.getString(cursor
	...
	public boolean queryAddress(String address) {
		String selectQuery = "SELECT  * FROM locations where snippet = '"
				+ address + "'";
		Cursor cursor = db.rawQuery(selectQuery, null);
		if (cursor.getCount() <= 0)
			return false; // doesn't exist
	...
	public ArrayList<String> getAddresses(String query, int group) {
		...
		if (group == 1)
			...
			cursor = db.rawQuery("SELECT * FROM locations WHERE groups LIKE '%"
					+ query + "%'", null);
		else
			cursor = db.rawQuery(
					"SELECT * FROM locations WHERE snippet LIKE '%" + query
							+ "%'", null);

		if (cursor.moveToFirst()) {
			do {
				addresses.add(cursor.getString(cursor
						.getColumnIndex(MySQLHelper.SNIPPET)));
			} while (cursor.moveToNext());
		}
