...
public class GroupDataManager {
	...
	SQLiteDatabase db;
	String[] cols = { MySQLHelper.GROUP_NAME, MySQLHelper.GROUP_DESC,
			MySQLHelper.GROUP_TYPE, MySQLHelper.GROUP_STATUS };
	...
	public String composegroupJSONfromSQLite() {
		...
		String selectQuery = "SELECT  * FROM groups where group_status = '"
				+ "yes" + "'";
		Cursor cursor = db.rawQuery(selectQuery, null);
		if (cursor.moveToFirst()) {
			do {
				HashMap<String, String> map = new HashMap<String, String>();
				map.put(MySQLHelper.GROUP_NAME, cursor.getString(cursor
						.getColumnIndex(MySQLHelper.GROUP_NAME)));
				wordList.add(map);
	...
	public List<String> getAllGroups() {
		...
		Cursor cursor = db.query(MySQLHelper.GROUP_TABLE, cols, null, null,
				null, null, null);
		cursor.moveToFirst();
	...
	public List<String> getJoinedGroups() {
		...
		cursor.moveToFirst();
		while (!cursor.isAfterLast()) {
			String g = cursor.getString(cursor
					.getColumnIndex(MySQLHelper.GROUP_NAME));
			groups.add(g);
			cursor.moveToNext();
		}
	...
	public List<String> getGroups(int j) {
		...
		String selectQuery;
		if (j == 0)
			selectQuery = "SELECT  * FROM groups where group_status = '"
					+ "yes" + "' ";
		else
	...
	public ArrayList<String> getGroups(String query) {
		...
		Cursor cursor = db.rawQuery(
				"SELECT * FROM groups WHERE group_name LIKE '%" + query + "%'",
				null);
	...
	public List<String> getDetails(String group) {
		...
		String selectQuery = "SELECT  * FROM groups where group_name = '"
				+ group + "'";
		Cursor cursor = db.rawQuery(selectQuery, null);
	...
	public void updateStatus(String group, String status) {
		String updateQuery = "Update groups set group_status = '" + status
	...
	public boolean queryStatus(String group) {
		...
		String selectQuery = "SELECT  * FROM groups where group_name = '"
				+ group + "'";
		Cursor cursor = db.rawQuery(selectQuery, null);
		// TODO make it dependent on searching the string instead of the the
		// position
		cursor.moveToFirst();
		if (cursor.getString(cursor.getColumnIndex(MySQLHelper.GROUP_STATUS))
	...
	public boolean isPrivate(String group) {
		...
		// position
		cursor.moveToFirst();
		if (cursor.getString(cursor.getColumnIndex(MySQLHelper.GROUP_TYPE))
