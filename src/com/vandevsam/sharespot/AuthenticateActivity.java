...
public class AuthenticateActivity extends Activity {
	...
	public void passLogin(View view) {
		...
		client.post("http://" + webServer + "/b/phpfiles/phplogin/login.php",
				params, new AsyncHttpResponseHandler() {
					...
					public void onSuccess(String response) {
						...
						try {
							...
							if (jObject.getBoolean("status")) {
								session.loginSession(jObject.getString("name"),
										username, jObject.getString("date"));
					...
					public void onFailure(int statusCode, Throwable error,
						...
						if (statusCode == 404) {
							Toast.makeText(getApplicationContext(),
									"Usernmae and/or password incorrect",
									Toast.LENGTH_LONG).show();
	...
	public void loadGroups() {
		...
		ServerUtilFunctions list = new ServerUtilFunctions(this,
				"Searching avaiable groups....");
		list.listAllGroup(user.get(SessionManager.KEY_USERNAME));
		Toast.makeText(getApplicationContext(),
				"Welcome " + user.get(SessionManager.KEY_NAME),
				Toast.LENGTH_LONG).show();
		loadMainActivity();
	}
