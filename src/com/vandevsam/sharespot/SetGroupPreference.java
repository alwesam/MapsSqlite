...
public class SetGroupPreference extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// TODO fix later
		/**
		 * try { SaveGroupPreference pref = new SaveGroupPreference(this);
		 * List<Boolean> group_check = pref.getPrefCheck();
		 * 
		 * Toast.makeText(getBaseContext(), group_check.size(),
		 * Toast.LENGTH_LONG).show();
		 * 
		 * //all checked true by default for (int i = 0; i < grCount; i++) {
		 * gridView.setItemChecked(i, group_check.get(i)); } } catch
		 * (NotFoundException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); Toast.makeText(getBaseContext(), "Empty List",
		 * Toast.LENGTH_LONG).show(); }
		 **/
